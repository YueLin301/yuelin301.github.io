...
If the agent has access to the environment's model , given $a_{t-1}, o_{t}$, and $b(\cdot\mid a_{t-1}, o_{t})$, then

$$
\begin{aligned}
b(s_{t}\mid o_{t}, a_{t-1}) 
    &= \frac{
        P(o_{t}\mid s_t, a_{t-1})\cdot P(s_t\mid a_{t-1})
    }
    {
        P(o_{t}\mid a_{t-1})
    }\\
