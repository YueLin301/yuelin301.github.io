...
One step of the process is: $(s_{t}, o_{t})\to a_{t}\to (s_{t+1} \to (o_{t+1}, r_{t}))$. At each timestep $t$,  
- each agent takes an action $a_t^i\in A^i$ based on its belief of the current state, given its observable $o_t^i$ and privious belief (the term "belief" will be introduced later),
- $s_{t+1}\sim \mathcal{P}(\cdot \mid s_t, a_t)$, 
- $o_{t+1} \sim \mathcal{Q}(\cdot \mid s_{t+1}, a_{t})$ and a reward is generated for **the whole team** based on the reward function $R(s,a)$.
...
> This definition is adapted from that of Wikipedia[^wiki-POMDP]. And I found its original definition is a bit of confusing, for agent observing $o$ after reaching $s'$. I suppose that $o$ is $o_{t+1}$ or $o'$.
{: .prompt-info }
...
Consider a two-player game. Each agent makes decisions based on its belief of the other's policy. So the two agents' polices are recursively dependant:
...
And $\mathcal{R}(b, a) = \sum\limits_{s} b(s) \cdot R(s,a)$. Compared to the original POMDP, the corresponding belief MDP is not partially observable anymore.
