...
1. **Reliability** — accuracy/consistency of measurement (how much of the score is signal versus error).
    ...
    - **Standard error of measurement (SEM; 测量标准误)** = $\sigma\_X \sqrt{1 - r\_{xx}}$; defines the confidence band around an individual's observed score and is the practical face of reliability.

2. **Stability** — **test–retest correlation (同一批被试在两个时点作答的相关; 重测相关)** over a specified time interval.
